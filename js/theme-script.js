(function($) {
    ...
    $(function() {
        ...
        $(window).scroll(function() {
            $('.cmc-navbar--sticky').each(function() {
                var method = $(window).scrollTop() > 10 ? 'addClass' : 'removeClass';
                $(this)[method]('cmc-navbar--stuck')
                    .not('.cmc-navbar--open')[method]('cmc-navbar--short');
            });
        });
        ...
        if ($.isMobile() && navigator.userAgent.match(/Chrome/i)) { // simple fix for Chrome's scrolling
            (function(width, height) {
                ...
                $(window).smartresize(function() {
                    ...
                    $('.cmc-section--full-height').css('height', windowHeight + 'px');
                });
            })($(window).width(), $(window).height());
        } else if (!isSupportViewportUnits) { // fallback for .cmc-section--full-height
            $(window).smartresize(function() {
                $('.cmc-section--full-height').css('height', $(window).height() + 'px');
            });
            $(document).on('add.cards', function(event) {
                if ($('html').hasClass('cmc-site-loaded') && $(event.target).outerFind('.cmc-section--full-height').length)
                    $(window).resize();
            });
        }

        // .cmc-section--16by9 (16 by 9 blocks autoheight)
        function calculate16by9() {
            $(this).css('height', $(this).parent().width() * 9 / 16);
        }
        $(window).smartresize(function() {
            $('.cmc-section--16by9').each(calculate16by9);
        });
        $(document).on('add.cards changeParameter.cards', function(event) {
            var enabled = $(event.target).outerFind('.cmc-section--16by9');
            if (enabled.length) {
                enabled
        ...
        // .cmc-parallax-background
        function initParallax(card) {
            setTimeout(function() {
                $(card).outerFind('.cmc-parallax-background')
                    .jarallax({
                        speed: 0.6
                    })
        ...
        if ($.fn.jarallax && !$.isMobile()) {
            $(window).on('update.parallax', function(event) {
                setTimeout(function() {
                    var $jarallax = $('.cmc-parallax-background');
        ...
        $(document).on('add.cards delete.cards', function(event) {
            ...
            fixedTopTimeout = setTimeout(function() {
                ...
                $('.cmc-fixed-top:first').each(function() {
                    fixedTop = {
        ...
        // embedded videos
        $(window).smartresize(function() {
            $('.cmc-embedded-video').each(function() {
                $(this).height(
                    $(this).width() *
        ...
        $(document).on('add.cards', function(event) {
            if ($('html').hasClass('cmc-site-loaded') && $(event.target).outerFind('iframe').length)
                $(window).resize();
        });
        ...
        function videoParser(card) {
            $(card).outerFind('[data-bg-video]').each(function() {
                ...
                var $img = $('<div class="cmc-background-video-preview">')
                    .hide()
                    .css({
                ...
                if (parsedUrl && (/youtube/g.test(parsedUrl[3]) || /vimeo/g.test(parsedUrl[3]))) {
                    ...
                    if (parsedUrl && /youtube/g.test(parsedUrl[3])) {
                        ...
                        if ($.fn.YTPlayer && !isBuilder && !$.isMobile()) {
                            $('> *:eq(1)', this).before('<div class="cmc-background-video"></div>').prev()
                                .YTPlayer({
        ...
        $(document).on('changeParameter.cards', function(event, paramName, value, key) {
            if (paramName === 'bg') {
                switch (key) {
                    case 'type':
                        $(event.target).find('.cmc-background-video-preview').remove();
                        if (value.type === 'video') {
                            videoParser(event.target);
                        }
                        break;
                    ...
                    do {
                        if (target.hash) {
                            ...
                            $(useBody ? 'body' : target.hash).each(function() {
                                ...
                                var stickyMenuHeight = $('.cmc-navbar--sticky').length ? 64 : 0;
        ...
        $('.cols-same-height .cmc-figure').each(function() {
            var $imageCont = $(this);
    ...
    if (!isBuilder) {
        ...
        if ($.fn.socialLikes) {
            $(document).on('add.cards', function(event) {
                $(event.target).outerFind('.cmc-social-likes').on('counter.social-likes', function(event, service, counter) {
        ...
        $(document).on('add.cards', function(event) {
            if ($(event.target).hasClass('cmc-reveal')) {
        ...
        $(document).ready(function() {
            ...
            if ($.isMobile()) {
                ...
                var $animatedElements = $('p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .cmc-author-name, em, label, input, textarea, .input-group, .iconbox, .btn-social, .cmc-figure, .cmc-map, .cmc-testimonial .card-block, .cmc-price-value, .cmc-price-figure, .dataTable, .dataTables_info').not(function() {
                    return $(this).parents().is('.navbar, .cmc-arrow, footer, .iconbox, .cmc-slider, .testimonial-section, .cmc-gallery, .cmc-testimonial .card-block, #cookiesdirective, .cmc-wowslider, .accordion, .tab-content, .engine, #scrollToTop, .partner-section, .cid-qyS5nU6OLV, .modal');
    ...
    if ($('nav.navbar').length) {
        ...
        $('.cmc-after-navbar.cmc-fullscreen').css('padding-top', navHeight + 'px');
    }
    ...
    if (!isBuilder && isIE()) {
        $(document).on('add.cards', function(event) {
            ...
            if ($eventTarget.hasClass('cmc-fullscreen')) {
            ...
            if ($eventTarget.hasClass('cmc-slider') || $eventTarget.hasClass('cmc-gallery')) {
                $eventTarget.find('.carousel-indicators').addClass('ie-fix').find('li').css({
                    display: 'inline-block',
                    width: '30px'
                });

                if ($eventTarget.hasClass('cmc-slider')) {
            ...
            $(this).addClass('countdown-inited').countdown($(this).attr('data-due-date'), function(event) {
                ...
                $(this).html(
                    event.strftime([
                        ...
                        '<span cmc-text class="period display-7">',$days,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span cmc-text class="period display-7">',$hours,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span cmc-text class="period display-7">',$minutes,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span cmc-text class="period display-7">',$seconds,'</span>',
                        '</span>',
                        '</div>',
                        '</div>'
                    ].join(''))
    ...
    if (isBuilder) {
        $(document).on('add.cards', function(event) {
            ...
            $(event.target).find('.carousel-item [cmc-media]').on('click', function(e) {
                clickHandler(e);
            });
            $(event.target).on('slide.bs.carousel', function() {
                $(event.target).find('.carousel-item .clonedCol [cmc-media]').off('click').on('click', function(e) {
                            clickHandler(e);
                        });
            });
        }).on('changeParameter.cards', function(event, paramName,value) {
